    ...
    <>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-[#E0E0E0] flex items-end">
                ...
                {({ isActive }) => (
                  <div className={cn(
                    ...
                    <div className={cn(
                      ...
                      isActive ? "bg-[#4a1020]" : "bg-[#6B1A2A]"
                    )}>
                      <item.icon size={24} strokeWidth={2} className="text-white" />
                    </div>
                    <span className={cn(
                      "text-[11px] font-semibold mt-1",
                      isActive ? "text-[#6B1A2A]" : "text-[#9B9B9B]"
                    )}>
                      {item.label}
                    </span>
                  </div>
        ...
        <button
          ...
          className={cn(
            "flex-1 flex flex-col items-center justify-center gap-0.5 py-2 text-[11px] font-medium transition-colors",
            showMore ? "text-[#6B1A2A]" : "text-[#9B9B9B]"
          )}
        >
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-full bg-[#F0F0F0] flex items-center justify-center">
                  <span className="text-sm font-bold text-[#6B1A2A]">
                    {user?.name?.charAt(0).toUpperCase()}
                  </span>
                </div>
                <div>
                  <p className="text-sm font-semibold text-[#6B1A2A]">{user?.name}</p>
