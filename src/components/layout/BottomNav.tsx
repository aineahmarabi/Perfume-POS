    ...
    <>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-[#E0E0E0] flex items-end">
                ...
                {({ isActive }) => (
                  <div className={cn(
                    ...
                    <div className={cn(
                      ...
                      isActive ? "bg-[#4e4369]" : "bg-[#685b8a]"
                    )}>
                      <item.icon size={24} strokeWidth={2} className="text-white" />
                    </div>
                    <span className={cn(
                      "text-[11px] font-semibold mt-1",
                      isActive ? "text-[#685b8a]" : "text-[#9B9B9B]"
                    )}>
                      {item.label}
                    </span>
                  </div>
        ...
        <button
          ...
          className={cn(
            "flex-1 flex flex-col items-center justify-center gap-0.5 py-2 text-[11px] font-medium transition-colors",
            showMore ? "text-[#685b8a]" : "text-[#9B9B9B]"
          )}
        >
          ...
          <div className="relative bg-white rounded-t-2xl shadow-lg pt-2 pb-6 px-4">
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-full bg-[#685b8a] flex items-center justify-center">
                  <span className="text-sm font-bold text-white">
                    {user?.name?.charAt(0).toUpperCase()}
                  </span>
                </div>
                <div>
                  <p className="text-sm font-semibold text-[#685b8a]">{user?.name}</p>
