        ...
        <div className="md:col-span-2 bg-white border border-[#E0E0E0] rounded-md p-4">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={stats.revenueByDay} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
              ...
              />
              <Bar dataKey="revenue" fill="#8B5A2B" radius={[2, 2, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
              ...
              return (
                <div key={method}>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="capitalize text-[#8B5A2B]">{method}</span>
                    <span className="font-mono text-[#8B5A2B]">{formatCurrencyShort(amount)}</span>
                  </div>
                  <div className="h-1.5 bg-[#F0F0F0] rounded-full">
                    <div
                      className="h-1.5 bg-[#8B5A2B] rounded-full"
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                </div>
            ...
            <div className="space-y-2">
              {(lowStock ?? []).slice(0, 5).map((item) => (
                <div key={item._id} className="flex items-center justify-between py-1.5 border-b border-[#F0F0F0] last:border-0">
                  <div>
                    <p className="text-sm font-medium text-[#8B5A2B]">{item.productName} {item.sizeMl}ml</p>
            ...
            <div className="space-y-2">
              {(recentSales ?? []).slice(0, 5).map((sale) => (
                <div key={sale._id} className="flex items-center justify-between py-1.5 border-b border-[#F0F0F0] last:border-0">
                  <div>
                    <p className="text-sm font-medium text-[#8B5A2B]">{sale.saleNumber}</p>
